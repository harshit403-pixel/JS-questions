...
let addItemtoater = document.querySelector(".addtoaster")


function goout() {
    setTimeout(() => {
        addItemtoater.style.transform = "translateY(200%)"
        addItemtoater.style.opacity = "0"
        console.log("running");
...
form.addEventListener("submit", (e) => {
    ...
    addItemtoater.style.transform = "translateY(0%)";
    addItemtoater.style.opacity = "100";
    goout()
    localStorage.setItem("products", JSON.stringify(products));
    printCard()
...
card.addEventListener("click", (dets) => {
    if (dets.target.id && dets.target.textContent == "Add to Cart") {
        ...
        addItemtoater.textContent = "added to cart";
        addItemtoater.style.transform = "translateY(0%)";
        goout()
    }
    ...
    if (dets.target.id && dets.target.innerHTML == "Remove") {
        ...
        addItemtoater.textContent = "item removed";
        addItemtoater.style.transform = "translateY(0%)";
    }
